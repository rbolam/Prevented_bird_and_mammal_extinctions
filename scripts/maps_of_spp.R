...
cand <- read.csv("all_info_candidates.csv")
cand <- select(cand, Scientific.name, className, t1993, t2010, redlistCategory)


countries <- read.csv("rl_data_29_01_2020/countries.csv")
...
cand$region <- noquote(cand$region) ## remove quote marks
cand$region <- str_trim(cand$region) ## remove
...
## Find col for scale_colour_manual:
colfunc <- colorRampPalette(c("#edf8b1", "#41b6c4", "#0c2c84"))
colfunc(5)
...
tiff("candidate_map.tiff", width = 7.08661, height = 7, res = 300, units = "in")
plot <- grid.arrange(plotacand, plotbcand, heights = c(1.07, 1))
dev.off()
jpeg("candidate_map.jpg", width = 7.08661, height = 7, res = 300, units = "in")
plot <- grid.arrange(plotacand, plotbcand, heights = c(1.07, 1))
dev.off()
...
tiff("1993_map.tiff", width = 7.08661, height = 7, res = 300, units = "in")
plot <- grid.arrange(plota1993, plotb1993, heights = c(1.07, 1))
dev.off()

jpeg("1993_map.jpg", width = 7.08661, height = 7, res = 300, units = "in")
plot <- grid.arrange(plota1993, plotb1993, heights = c(1.07, 1))
dev.off()
...
tiff("2010_map.tiff", width = 7.08661, height = 7, res = 300, units = "in")
plot <- grid.arrange(plota2010, plotb2010, heights = c(1.07, 1))
dev.off()

jpeg("2010_map.jpg", width = 7.08661, height = 7, res = 300, units = "in")
plot <- grid.arrange(plota2010, plotb2010, heights = c(1.07, 1))
dev.off()
